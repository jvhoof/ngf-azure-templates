...
echo ""
echo "--> Creating $rg_cuda resource group ..."
az group create --location "$location" --name "$rg_cuda"

# Validate template
echo "--> Validation deployment in $rg_cuda resource group ..."
az group deployment validate --resource-group "$rg_cuda" \
...
echo "--> Deployment of $rg_cuda resources ..."
az group deployment create --resource-group "$rg_cuda" \
                           --template-file azuredeploy.json \
                           --parameters "@azuredeploy.parameters.json" \
                           --parameters adminPassword=$passwd prefix=$prefix
result=$? 
